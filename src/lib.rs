...
const ARBITRARY12: u64 = 0xba7c9045f12c7f99;
const ARBITRARY13: u64 = 0x24a19947b3916cf7;
...
#[inline(always)]
fn load(bytes: &[u8], offset: usize) -> u64 {
    unsafe { bytes.as_ptr().add(offset).cast::<u64>().read_unaligned() }
    // u64::from_ne_bytes(unsafe { bytes.get_unchecked(offset..offset + 8).try_into().unwrap() })
}
...
#[inline(always)]
fn hash_bytes_short(bytes: &[u8], accumulator: u64, seeds: &[u64; 8]) -> u64 {
    let len = bytes.len();
...
#[cold]
#[inline(never)]
fn hash_bytes_long(mut v: &[u8], accumulator: u64, seeds: &[u64; 8]) -> u64 {
    let mut s0 = accumulator;
